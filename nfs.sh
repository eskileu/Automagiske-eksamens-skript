...
## 
# Automatisk oppsett av NFS-tjener
...
apt-get install -qy nfs-kernel-server nfs-common portmap
...
SPORSMAL="Skriv inn navnet på sharet (default er /home):"
getInput 1
if [ -z $INPUT_LOWER_CASE ]; then
	SHARE="/home"
else
	SHARE=$INPUT_LOWER_CASE
fi
...
SPORSMAL="Skriv inn nettverksmaska (default er netmasken til eth0):"
getInput 1
if [ -z $INPUT_LOWER_CASE ]; then
	NETWORK=`/sbin/ifconfig eth0 | grep 'Mask' | mawk '{print $4}' | cut -d: -f2`
else
	NETWORK=$INPUT_LOWER_CASE
fi

rm /etc/exports

echo "
$SHARE $IPADDR/$NETWORK(rw,sync,no_root_squash,no_subtree_check)" > /etc/exports
